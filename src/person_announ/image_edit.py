...
def correct_dimension(dim: list):
    if dim is not None:
...
class ImageEditing:
    ...
    def blur_roi(self, blur_type: str, dim: list, magnitude: int = 15):
        if magnitude % 2 == 0:
            magnitude += 1

        x, y, width, height = correct_dimension(dim)
