...
filters_config = {
    'leather':
        [{'path': "filters/hard_leather.png",
          'anno_path': "filters/labels_leather.csv",
...
def getLandmarks(img):
    ...
    selected_keypoint_indices = [127, 93, 58, 136, 150, 149, 176, 148, 152, 377,
                                 400, 378, 379, 365, 288, 323, 356, 70, 63, 105,
                                 66, 55, 285, 296, 334, 293, 300, 168, 6, 195,
                                 4, 64, 60, 94, 290, 439, 33, 160, 158, 173,
                                 153, 144, 398, 385, 387, 466, 373, 380, 61, 40,
                                 39, 0, 269, 270, 291, 321, 405, 17, 181, 91,
                                 78, 81, 13, 311, 306, 402, 14, 178, 162, 54,
                                 67, 10, 297, 284, 389]
...
def load_filters(filter_name="dog"):
...
filters, multi_filter_runtime = load_filters(next(iter_filter_keys))
...
def main():
    ...
    while True:
        ...
        # print(getLandmarks(img))
        img = cv2.flip(img, 1)
        if success:
            image = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            image.flags.writeable = False

            normalized_results = face_mesh.process(image)
            if normalized_results.multi_face_landmarks:
                normalized_points = normalized_results.multi_face_landmarks[0].landmark
                result = []
                for index, point in enumerate(normalized_points):
                    # Изначально значения координат точек нормализованы в диапазоне [0, 1], поэтому мы изменяем их
                    point = [round(point.x * image.shape[1]), round(point.y * image.shape[0])]
                    result.append(point)
                # print(result)
...
#
# def mask(img: np.ndarray, mask: np.ndarray) -> np.ndarray:
    ...
    while True:
        ...
        if faces is not None:
            for iteration, face in enumerate(faces):
                ...
                img_edit.blur_roi(blur_type=blur_type,
                                  dim=face_bbox,
                                  magnitude=magnitude)
